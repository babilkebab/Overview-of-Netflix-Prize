...
from models import MiniTimeSVDpp
import sys
import os
...
if __name__ == "__main__":
    ...
    train_tensor_X = torch.tensor(df_train[["user", "item", "bin"]].values)
    train_tensor_Y = torch.tensor(df_train["label"].values, dtype=torch.float32)
    probe_tensor_X = torch.tensor(df_probe[["user", "item", "bin"]].values)
    ...
    model = MiniTimeSVDpp(n_items=17770, n_users=480189, factors=K, training_global_avg=global_avg, user_rated_movies=users_rated_movies_dict, user_inv_sqrt=users_inv_sqrt_dict)
    ...
    MODEL_PATH = f"models/minitimesvdpp_model{K}.pth"
