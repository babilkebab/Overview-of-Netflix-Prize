...
def compute_cf_probe_set(top_k):
    ...
    for movie in probe_data.keys():
        for user in probe_data[movie]:
            ...
            predicted_ratings.append(norm_itemitem_collaborative_filtering(user, movie, top_k))
...
def norm_itemitem_collaborative_filtering(user_id, movie_id, top_k):
    global norm_all_similarities_movies_no_probe, csr_no_probe
    ...
    rating = weighted_avg(top_ratings, top_sims)
